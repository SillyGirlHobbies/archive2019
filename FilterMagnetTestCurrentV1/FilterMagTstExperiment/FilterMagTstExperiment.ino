// this software is for the Audeze 9 magnet headphone planar magnetic array
#include "TimerOne.h"
...
{ // this function lights three out of nine LEDs each time it is called
  ...
  if ( led_group == 1 ) 
  {
    ...
    for( m = 1; m <= 3; m++ )
    {
      if (leds[m] == 0) {  digitalWrite( red[m],  LOW);  digitalWrite(grn[m],  LOW); }
      if (leds[m] == 1) {  digitalWrite( red[m], HIGH);  digitalWrite(grn[m],  LOW); }
      if (leds[m] == 2) {  digitalWrite( red[m],  LOW);  digitalWrite(grn[m], HIGH); }
    }
    led_group = 2;
    return;
  }
...
void loop() // test for all magnets same orientation
{
  ...
  hall[1] = analogRead(A0);
  hall[2] = analogRead(A2);  
  hall[3] = analogRead(A5);
  hall[4] = analogRead(A3);  
  hall[5] = analogRead(A1);
  hall[6] = analogRead(A7); 
  hall[7] = analogRead(A6);
  //digitalWrite(ASWITCHSEL,0); // A0 analog channel is muxed between two Hall sensors
  //delayMicroseconds(50);
  hall[8] = analogRead(A4);
